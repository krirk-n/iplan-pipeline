...
for (user in uniqueUsers_filtered) {
  b_plot <- ggplot(submission_detailed_df %>% filter(userKey == user), aes(x = factor(submissionCount))) +
    geom_bar(aes(y = businessMax), stat = "identity", fill = "lightblue", position = "dodge") +
    geom_bar(aes(y = business), stat = "identity", fill = "blue", position = "dodge") +
    labs(title = "Business",
         x = "submissionCount",
         y = "count",
         fill = "Variable") +
    theme_minimal() +
    geom_text(aes(y = businessMax, label = businessMax), vjust = -0.5) +
    geom_text(aes(y = business, label = business), vjust = 1.5)
  
  e_plot <- ggplot(submission_detailed_df %>% filter(userKey == user), aes(x = factor(submissionCount))) +
    geom_bar(aes(y = environmentalMax), stat = "identity", fill = "lightgreen", position = "dodge") +
    geom_bar(aes(y = environmental), stat = "identity", fill = "forestgreen", position = "dodge") +
    labs(title = "Environmental",
         x = "submissionCount",
         y = "count",
         fill = "Variable") +
    theme_minimal() +
    geom_text(aes(y = environmentalMax, label = environmentalMax), vjust = -0.5) +
    geom_text(aes(y = environmental, label = environmental), vjust = 1.5)
  
  c_plot <- ggplot(submission_detailed_df %>% filter(userKey == user), aes(x = factor(submissionCount))) +
    geom_bar(aes(y = communityMax), stat = "identity", fill = "peachpuff", position = "dodge") +
    geom_bar(aes(y = community), stat = "identity", fill = "darkorange", position = "dodge") +
    labs(title = "Community",
         x = "submissionCount",
         y = "count",
         fill = "Variable") +
    theme_minimal() +
    geom_text(aes(y = communityMax, label = communityMax), vjust = -0.5) +
    geom_text(aes(y = community, label = community), vjust = 1.5)
