...
```{r}
userkey = "allisonceceri" # Change as appropriate
...
codes <- c("Commercial", "Conservation_lu", "Cropland", "Industrial", "Pasture", "Recreation", "Residential HD", "Residential LD", "Timber", "Wetlands")
...
for (i in 1:length(set$line.weights$user_name)) {
  if (set$line.weights$user_name[i] == userkey) {
    ...
    matuser <- ggplot(matrix_long, aes(x=Var1, y=Var2, fill=value)) +
      ...
      labs(x="", y="", title= paste(userkey, "submission number", str_sub(submission, -3, -1)), fill="Intensity")
    print(matuser)
  }
}
```
