...
print(uniqueUsers)
```
Plot the distribution of approval count for the last submission for each user
```{r}
last_subs <- submission_df %>%
  group_by(userKey) %>%
...
```{r}
ggplot(last_subs, aes(x = approvalCount)) +
  geom_histogram(binwidth = 1, fill = "#f5b962", color = "#a37f4c") +
  labs(
    title = "Histogram of Approval Count for Last Submission",
    x = "Approval Count",
    y = "Frequency"
  ) +
  theme_minimal()

ggplot(submission_df, aes(x = approvalCount)) +
  geom_histogram(binwidth = 1, fill = "#f5b962", color = "#a37f4c") +
  labs(
    title = "Histogram of Approval Count for All Submissions",
    x = "Approval Count",
    y = "Frequency"
  ) +
  theme_minimal()
...
```{r}
userkey = "asher" # Change as appropriate
...
for (i in subs) {
  ...
  units(
      ...
      style(name = "submission point", traces = c(2))
  ona_plots[[i]] <- p
}
```
...
node_space <- ona:::plot.ena.ordered.set(set, title = "Last Submission for Each Player") %>%
  ...
  units(
      ...
      points_color = "green",
      point_position_multiplier = point_position_multiplier,
      show_mean = TRUE, show_points = FALSE, with_ci = F) %>%
  units(
...
node_space %>%
  units(
    points = set$points %>% filter(user_name=="asher"),
    points_color = "#8658d5",
    point_position_multiplier = 2,
    show_mean = F, show_points = TRUE, with_ci = F) %>%
  units(
    points = set$points %>% filter(user_name=="forrest"),
    points_color = "#fba2d0",
    point_position_multiplier = 2,
    show_mean = F, show_points = TRUE, with_ci = F) %>%
  units(
    points = set$points %>% filter(user_name=="julian"),
    points_color = "#92b9e3",
    point_position_multiplier = 2,
    show_mean = F, show_points = TRUE, with_ci = F) %>%
  units(
    points = set$points %>% filter(user_name=="chloe"),
    points_color = "#5fa777",
    point_position_multiplier = 2,
    show_mean = F, show_points = TRUE, with_ci = F) %>%
  plotly::layout(showlegend = TRUE, legend = list(x = 100, y = 0.9)) %>% 
    style(name = "asher", traces = c(11)) %>% 
    style(name = "forrest", traces = c(12)) %>% 
    style(name = "julian", traces = c(13)) %>%
    style(name = "chloe", traces = c(14))
```
...
result_list <- list()
for (i in 1:146) {
  result_list[[i]] <- paste("Index:", i, "Userkey:", sub_result[[i]]$userKey)
}

indices <- list()
for (i in 1:length(result_list)) {
  if (grepl("Userkey: chloe", result_list[[i]])) {
    indices <- c(indices, i)
  }
}
indices
sub_result[[12]]$stakeholderApproval
```
...
ona:::plot.ena.ordered.set(set, title = paste(userkey, "submission nodes")) %>%
  units(
      ...
      points_color = "green",
      point_position_multiplier = point_position_multiplier,
      show_mean = TRUE, show_points = FALSE, with_ci = T) %>%
  units(
      points = setA$points[SH == sh_name[2] & Satisfy == "Yes"],
      points_color = "blue",
      point_position_multiplier = point_position_multiplier,
